...
| Setting | Default | Effect |
|---|---|---|
| `mergeNB.security.trustContent` | tracks [Workspace Trust](https://code.visualstudio.com/docs/editor/workspace-trust) | Renders notebook-authored content (markdown HTML, rich cell outputs) as trusted, but only inside a workspace you've already granted Workspace Trust to. Defaults to `true` when the workspace is trusted and `false` otherwise; set it explicitly to override that. |
