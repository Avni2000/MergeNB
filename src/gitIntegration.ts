 ...
 */
export async function getUnmergedFiles(workspaceFolder?: vscode.WorkspaceFolder): Promise<GitFileStatus[]> {
    try {
        const gitRoot = workspaceFolder?.uri.fsPath || vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!gitRoot) return [];
